...
export const MODRINTH_DOWN_SHORT =
  "Modrinth's mod data is down right now, so this is paused. It's not your connection or our site. Please try again in a few minutes.";
 ...
 * becomes a clearly disabled chip plus a visible note (tooltips don't exist on
 * touch), because a randomized pack we can't verify is what we promise never to ship.
 */
export default function LuckyButton({ className = "btn-ghost" }: { className?: string }) {
  ...
  if (up === false) {
    return (
      <>
        <span className={`${className} is-disabled`} role="button" aria-disabled="true" title={MODRINTH_DOWN_BODY}>
          <Icon name="dice" size={15} /> Feeling lucky · unavailable
        </span>
        <span className="lucky-down-note">{MODRINTH_DOWN_SHORT}</span>
      </>
    );
  }
